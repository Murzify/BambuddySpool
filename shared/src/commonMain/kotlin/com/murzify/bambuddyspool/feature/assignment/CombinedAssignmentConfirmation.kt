...
fun AssignmentContext.toCombinedConfirmation(): CombinedAssignmentConfirmation {
    val printerNames = printers.associate { it.id to (it.name ?: it.id.value.toString()) }
    ...
    return CombinedAssignmentConfirmation(
        spoolName = spool.name ?: "Spool ${spool.id.value}",
        spoolId = spool.id.value,
        currentLocations = current,
        target = AssignmentLocation(
            printerName = targetPrinter.name ?: targetPrinter.id.value.toString(),
    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                Text(stringResource(Res.string.assignment_spool, confirmation.spoolName, confirmation.spoolId))
                ...
                Text(stringResource(Res.string.assignment_target_printer, confirmation.target.printerName))
...
private fun AssignmentLocation.displayText(): String = "$printerName — $slotLabel"
