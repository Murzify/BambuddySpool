@file:Suppress("FunctionNaming") // Compose functions use UpperCamelCase by convention.
...
import com.murzify.bambuddyspool.shared.resources.placeholder_spools
        ...
        BoxWithConstraints(modifier = Modifier.fillMaxSize()) {
            if (maxWidth < 600.dp) {
                Column(modifier = Modifier.fillMaxSize()) {
                    RootContent(state = state, modifier = Modifier.weight(1f))
...
@Composable
private fun RootContent(state: RootState, modifier: Modifier) = Surface(modifier = modifier.fillMaxSize()) {
    Column(modifier = Modifier.fillMaxSize().padding(24.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        when (state.destination) {
            RootDestination.Home -> HomeScreen(state)
            RootDestination.Spools -> Text(stringResource(Res.string.placeholder_spools))
