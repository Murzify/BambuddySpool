...
- Removed unused printer activity state, per-row persistence timestamps, assignment creation timestamps, normalized spool copies, and indexes that did not support active queries.
- Preserved `schema-v1.sql` as the immutable historical export. Bumped the active Room schema to v2, added `schema-v2.sql`, and added `BambuddyDatabaseMigrations.MIGRATION_1_2` to rebuild tables while copying all retained snapshot data before dropping v1 tables. Room KMP database builders must register `BambuddyDatabaseMigrations.all` through `addMigrations`.
...
- [x] The tracked version 1 schema export is retained unchanged; version 2 is the active minimized schema.
- [x] A deterministic Room KMP `1 -> 2` migration path preserves retained rows and rebuilds FTS before the active schema is opened.
